...
Matching flow at runtime (`src/allCardIndex.ts`): load `manifest.json` + `routing.json` (network-first via the service worker), then a **two-stage route selection** over all ~110k cards — a cheap hash/chroma prefilter trims to a generous shortlist, then the full `scoreRoute` correlations rank down to `ROUTE_SHORTLIST_SIZE` (600) candidates. Lazily fetch only the needed set shards (cached in the PWA cache), then fine-rank — first identify the card via artwork/title, then disambiguate printings via the print-specific regions. Top match plus two alternatives go to the UI for confirmation. Perf note: `hammingDistance` compares the 64-bit hashes as two 32-bit popcounts (not BigInt) — it runs millions of times per scan. A warm confident scan is ~0.5s; the first scan of a card whose set shards aren't cached yet pays the lazy shard fetch/parse.
