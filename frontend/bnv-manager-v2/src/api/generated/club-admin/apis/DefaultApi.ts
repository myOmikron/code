...
export class DefaultApi extends runtime.BaseAPI {
    ...
    async extendInviteExpiryRaw(requestParameters: ExtendInviteExpiryRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<FormResultForNullAndExtendInviteExpiryError>> {
        if (requestParameters['club_uuid'] == null) {
            throw new runtime.RequiredError(
                'club_uuid',
                'Required parameter "club_uuid" was null or undefined when calling extendInviteExpiry().'
            );
        }

        if (requestParameters['invite_uuid'] == null) {
            throw new runtime.RequiredError(
                'invite_uuid',
                'Required parameter "invite_uuid" was null or undefined when calling extendInviteExpiry().'
            );
        }
        ...
        const response = await this.request({
            path: `/api/v1/frontend/club-admin/clubs/{club_uuid}/invites/{invite_uuid}/extend-expiry`.replace(`{${"club_uuid"}}`, encodeURIComponent(String(requestParameters['club_uuid']))).replace(`{${"invite_uuid"}}`, encodeURIComponent(String(requestParameters['invite_uuid']))),
            method: 'POST',
            headers: headerParameters,
            query: queryParameters,
