...
# i18next appends these to a key when it is pluralized, so `t("label.cards", { count })` is
# stored as `label.cards_one` / `label.cards_other`. The source only ever mentions the bare
# slug, so without this the scanner would keep re-adding it as an untranslated placeholder.
PLURAL_SUFFIXES = ("_zero", "_one", "_two", "_few", "_many", "_other")


def has_translation(group, inner_key):
    """
    Whether a key is present in a namespace group, verbatim or as an i18next plural variant.
    :param group: the outer key's dict, e.g. everything under "label"
    :param inner_key: the slug as it appears in the source
    :return: True if the key is already translated
    """
    if inner_key in group:
        return True
    return any(f"{inner_key}{suffix}" in group for suffix in PLURAL_SUFFIXES)


def base_slug(inner_key):
    """
    The slug an i18next plural variant belongs to, e.g. "cards_one" -> "cards".
    :param inner_key: the slug as it appears in the translation file
    :return: the slug as it appears in the source
    """
    for suffix in PLURAL_SUFFIXES:
        if inner_key.endswith(suffix):
            return inner_key[: -len(suffix)]
    return inner_key
            ...
            if file_path.exists():
                for pattern in patterns:
                    ...
                    if len(pattern) == 2:
                        ...
                        if not has_translation(content[key], inner_key):
                            print(
                                Fore.LIGHTGREEN_EX
        ...
        for lang_dir in LOCAL_DIR.iterdir():
            if lang_dir.is_dir() and lang_dir.name in LANGUAGES:
                for file in lang_dir.iterdir():
                    ...
                    # A plural variant is used whenever its bare slug appears in the source
                    used = self.translations[file.stem]
                    difference = {
                        (key, inner_key)
                        for key, inner_key in file_set
                        if (key, inner_key) not in used
                        and (inner_key is None or (key, base_slug(inner_key)) not in used)
                    }
    ...
    for lang_dir in LOCAL_DIR.iterdir():
        if lang_dir.is_dir() and lang_dir.name in LANGUAGES:
            for file_path in lang_dir.iterdir():
                ...
                with open(file_path, "w", encoding="utf-8") as f:
                    json.dump(translations, f, indent=4, sort_keys=True, ensure_ascii=False)
