...
//! reference path (hybrid-scan, exercised by the regression harness) and the split
//! worker/main-thread app path (scan-worker + scan-client). Keeping the policy here avoids
...
/**
 * Which signal identified the card
 */
export type ScanMethod = "perceptual" | "ocr";

/**
 * Whether perceptual matching alone is trustworthy enough to skip OCR
 *
 * @param matches
 * @returns whether OCR can be skipped
 */
...
/**
 * The candidates a recognised card name resolved to, with that name's score
 */
 ...
 * @param perceptual
 * @param title
 * @returns the candidates to show and which signal produced them
 */
