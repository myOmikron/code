...
/**
 * The Tesseract worker type, as its factory returns it
 */
...
/**
 * The OCR worker, started on first use
 *
 * @returns
 */
...
/**
 * Fills `out` with the luma of every pixel
 *
 * @param pixels
 * @param out
 */
 ...
 *
 * @param source
 * @param bandSearchFraction
 * @returns
 */
async function titleCropBlobs(
    source: CanvasImageSource,
    bandSearchFraction: number,
): Promise<{ blobs: Blob[]; band: [number, number] | null }> {
 ...
 * @param options.isConclusive
 * @param options.bandSearchFraction
 * @returns the recognised title text
 */
