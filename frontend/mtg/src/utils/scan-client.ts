...
import type { AllCardIndexSummary } from "./all-card-index";
import { decideMatches, OCR_NAME_MIN, OCR_NAME_MIN_CORROBORATED, OCR_NAME_TRUST } from "./hybrid-decision";
import type { CardQuad, ScanOverlay } from "./image-hash";
import type { ScanProfile } from "./scan-worker";
import { matchBasicLandName, matchCardName } from "./name-index";
import { recognizeCardTitle } from "./title-ocr";
import type { CardRecord, ImageSignature, MatchCandidate } from "src/types";

export type { ScanOverlay, CardQuad } from "./image-hash";
...
/**
 * The crops OCR should read, handed over as soon as they exist
 */
type TitleReadyMessage = { type: "title-ready"; id: number; titleBitmap: ImageBitmap; fullArtNameBitmap: ImageBitmap };
/**
 * A message that reports progress rather than a result
 */
...
/**
 * Anything a pending request may resolve to
 */
...
/**
 * The scan worker, started on first use
 *
 * @returns
 */
 ...
 *
 * @param onProgress
 * @returns what the app needs to know about the loaded index
 */
 ...
 *
 * @param name
 * @returns every printing of that card
 */
...
/**
 * A scan result plus which signals identified the card
 */
...
async function readTitle(
    ...
    try {
        ...
        const banner = await recognizeCardTitle(fullArtNameBitmap, {
            onProgress: onOcrProgress,
            bandSearchFraction: 1,
        });
 ...
 * @param ocrTimeoutMs
 * @param setCodes
 * @returns the result plus which signals identified the card
 */
export async function scanImage(
    ...
    const scan = await request<ScannedMessage>({ type: "scan", blob, setCodes }, undefined, (stage) => {
        if (stage.type === "title-ready") {
            ...
            pendingOcr.promise = readTitle(stage.titleBitmap, stage.fullArtNameBitmap, (fraction) =>
                emit("reading", { ocr: fraction }),
            );
