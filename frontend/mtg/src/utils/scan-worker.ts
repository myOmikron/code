...
//! importScripts (unavailable in a module worker), so scan-client runs it on the main thread
...
import { isConfident } from "./hybrid-decision";
import {
    createFullArtNameOcrSource,
    createScanOverlay,
    createScanSignatures,
    createTitleOcrSource,
} from "./image-hash";
import type { ScanOverlay } from "./image-hash";
...
worker.onmessage = async (event) => {
    ...
    try {
        ...
        if (message.type === "match-title") {
            const result = await findMatchesByTitle(
                message.ocrText,
                message.signatures,
                3,
                await resolveSetFilter(message.setCodes),
            );
            worker.postMessage({
                type: "title-matches",
                id: message.id,
        ...
        if (message.type === "list-printings") {
            ...
            worker.postMessage({
                type: "printings",
                id: message.id,
                printings: await listPrintingsByName(message.name),
            });
            return;
        }
        ...
        try {
            ...
            // OCR'd when the top strip yields no name (see scan-client.readTitle), but cutting it here
            ...
            worker.postMessage({ type: "title-ready", id: message.id, titleBitmap, fullArtNameBitmap }, [
                titleBitmap,
                fullArtNameBitmap,
            ]);
