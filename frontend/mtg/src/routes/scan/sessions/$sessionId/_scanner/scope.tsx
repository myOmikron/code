import { ChevronLeftIcon, ChevronRightIcon, MagnifyingGlassIcon, Square3Stack3DIcon } from "@heroicons/react/20/solid";
import { createFileRoute, useNavigate } from "@tanstack/react-router";
import { Alert, AlertDescription, AlertTitle, Button, HeadingLayout, PrimaryButton, Text } from "components";
...
export const Route = createFileRoute("/scan/sessions/$sessionId/_scanner/scope")({
    component: ScanScopeRoute,
});
...
function ScanScopeRoute() {
    ...
    function start(next: string[]) {
        choose(next);
        void navigate({ to: "/scan/sessions/$sessionId/live", params: { sessionId } });
    }

    return (
        <main className="min-h-svh bg-(--surface-page) px-5 py-8 lg:px-12 lg:py-10">
            <div className="lg:mx-auto lg:max-w-220">
                <HeadingLayout
                    heading={t("heading.what-to-scan")}
                    headingDescription={t("description.scope")}
                    headingChildren={
                        <Button
                            plain
                            onClick={() => void navigate({ to: "/scan/sessions/$sessionId", params: { sessionId } })}
                        >
                            <ChevronLeftIcon className="size-5" /> {t("button.to-session")}
                        </Button>
                    }
                >
                    {/* The count belongs beside the button, not inside it: a Button fills with its colour,
                    and Text's muted grey is unreadable on the filled primary. */}
                    <div className="mt-6 grid grid-cols-2 gap-4">
                        <div className="flex flex-col gap-2">
                            <PrimaryButton className="w-full" disabled={!ready} onClick={() => start([])}>
