...
import { DeckHandComposition } from "src/components/deck-hand-composition";
import { ManaCost } from "src/components/mana-cost";
import { useDeckFreeMulligan } from "src/utils/deck-mulligan-settings";
import type { DeckOdds, HandSplit, HandVerdict } from "src/utils/deck-odds";
import { HAND_SPLITS, verdictFor } from "src/utils/deck-odds";
import { formatChance } from "src/utils/format";
import type { Translate } from "src/utils/translate";

/** What the opening hand can be read as: its lands, or what it is made of */
type HandView = "lands" | HandSplit;

/** The views in the order they are offered */
const HAND_VIEWS: Array<HandView> = ["lands", ...HAND_SPLITS];
...
export type DeckOddsPanelProps = {
    ...
    odds: DeckOdds;
    /** The tags that exist, for naming and colouring the hand's tag rows */
 ...
 * @returns the panel
 */
export function DeckOddsPanel({ deckId, odds, tags }: DeckOddsPanelProps) {
    const [t] = useTranslation("deck");
    const [freeMulligan, setFreeMulligan] = useDeckFreeMulligan(deckId);
    const [view, setView] = useState<HandView>("lands");

    if (odds.opening.deckSize === 0) return null;

    const hand = freeMulligan ? odds.opening.mulliganed : odds.opening.first;
    const composition = freeMulligan ? odds.opening.composition.mulliganed : odds.opening.composition.first;
    // A deck nobody has tagged has nothing to show under that heading, so the
    // view is not offered at all rather than opened onto an empty list.
    const offered = HAND_VIEWS.filter((option) => option === "lands" || composition[option].length > 0);
    const chosen = offered.includes(view) ? view : "lands";
            ...
            >
                <div className={"flex items-start justify-between gap-4"}>
                    <div className={"min-w-0"}>
                        <h3 className={"text-sm/6 font-medium text-zinc-950 dark:text-white"}>
                            {t("heading.opening-hand")}
                        </h3>
                        <Text className={"mt-1 text-xs"}>
                            {t("description.opening-hand", {
                                lands: odds.opening.lands,
                                cards: odds.opening.deckSize,
                            })}
                        </Text>
                    </div>
                    {offered.length > 1 && (
                        <SplitToggle
                            ...
                            value={chosen}
                            onChange={setView}
                            nameOf={(option) => viewName(t, option)}
                        />
                    )}
                </div>
                ...
                {chosen !== "lands" ? (
                    <div className={"mt-4 flex flex-col gap-1.5"}>
                        <Text className={"text-xs"}>{t("description.hand-composition")}</Text>
                        <DeckHandComposition groups={composition[chosen]} split={chosen} tags={tags} t={t} />
                    </div>
                ) : (
                    <>
                        <div className={"mt-4 flex flex-col gap-1.5"}>
                                    ...
                                    <div key={entry.lands} className={"flex items-center gap-2"}>
                                        ...
                                        >
                                            <div
                                                ...
                                                style={{
                                                    width: `${Math.max(entry.chance * 100, entry.chance > 0 ? 1 : 0)}%`,
                                                }}
                                            />
                                        </div>
                        ...
                        <div
                            className={"mt-4 flex flex-col gap-3 border-t border-zinc-950/5 pt-4 dark:border-white/10"}
                        >
                        ...
                        <div className={"flex flex-wrap gap-3"}>
                            {odds.opening.colors.map((color) => (
                                <span key={color.key} className={"flex items-center gap-1.5"}>
                                    ...
                                    <Strong className={"text-xs tabular-nums"}>{formatChance(color.chance)}</Strong>
                                </span>
                            ))}
                        </div>
                    </div>
 ...
 *
 * Spelled out rather than assembled from the slug: the translation scanner only
 * reads keys written inside a translate call.
 *
 * @param t the deck namespace's translate function
 * @param view what the hand is read as
 *
 * @returns its name
 */
function viewName(t: Translate, view: HandView): string {
    switch (view) {
        case "lands":
            return t("label.lands");
        case "mana":
            return t("label.split-mana");
        case "tags":
    ...
    return (
        <div className={"flex flex-col gap-1"}>
            <div className={"flex items-baseline justify-between gap-3"}>
                ...
                <Strong className={"text-sm tabular-nums"}>{formatChance(chance)}</Strong>
            </div>
